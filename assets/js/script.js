...
let dataArray = [] //Guarda el array que se obtiene del servido
let myChart = null
let lastchange = 0;

async function ConectionServer() {
  try {
    let currency = selectTypeMoney.value;
    const res = await fetch(`https://mindicador.cl/api/${currency}`)
    const data = await res.json()
    let lastchange = (data.serie[0].valor);
    let test = (data.serie);
    calculo();
    let dataArray = data.serie // Aca es donde guardo el array original en la variable dataArray
